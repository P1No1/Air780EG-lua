...
sys.taskInit(function()
  ...
  local function read_data(channel)
    ...
    -- 读取数据
    local data = uart:rd_fifo(channel, uart.REG.RHR, 20)
    return data
  end
  ...
  local function process_data_2(data, channel)  -- 计算PM2.5改为温湿度
    ...
    -- 解析数据字节
    local pm25_high = string.byte(data, 1)  -- 温度 高字节
    local pm25_low = string.byte(data, 2)   -- 温度 低字节
  ...
  while true do
    ...
    local pm25A, pm10A = process_data_2(dataA, "A")
