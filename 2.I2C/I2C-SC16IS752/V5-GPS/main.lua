...
sys.taskInit(function()
  ...
  local function read_lot_data(channel)
    -- uart:wr_fifo(channel, uart.REG.THR, cmd_2)  -- 发送命令
    sys.wait(100)  -- 等待响应

    -- 读取数据
    local data = uart:rd_fifo(channel, uart.REG.RHR, 100)
    return data
  end
  ...
  local function process_data(data, channel)
    ...
    local check1 = string.byte(data, 1)    -- 帧头
    local check2 = string.byte(data, 2)    -- 帧头
    ...
    if pm25 >= 1 and pm25 ~= 2570 then    -- 此判断条件可根据实际情况调整
      return check1, check2, pm25, pm10
    else
      log.warn("i2c", "Invalid data received on channel " .. channel)
  ...
  while true do
    ...
    local check1, check2, pm25B, pm10B = process_data(dataB, "B")
    ...
    if check1 ~= 170 then
      log.info("i2c"..I2C_ID.."--uartB", "Bad PM2.5 data"..check1,check2,dataB:toHex())
      dataB = read_lot_data("B")
    end
...
uart.on(uartid, "receive", function(id, len)
    ...
    repeat  
        ...
        if #s > 0 then -- #s 是取字符串的长度
            ...
            log.info("uart", "receive", id, #s, s) --如果传输二进制/十六进制数据, 部分字符不可见, 不代表没收到
